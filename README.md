...
In write mode, click an editable matrix cell to open its desired-state editor. Click an outlined missing cell to use the unique fleet value or choose among tied variants, inspect the live operation plan, and fill the hole. Zone settings edit a desired value. DNS records edit a desired definition, with a record chooser when one DNS cell contains multiple records. Rule editors lead with structured name, enabled state, expression, action, phase, and action-specific fields; exact JSON remains available as the advanced editor for heterogeneous Cloudflare rule schemas. Use **Select targets** before running a fleet alignment or rule-copy action. **Edit settings** filters the matrix to zone settings and focuses the first editable cell.
