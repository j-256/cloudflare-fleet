...
export const retrievalOutputSchemas = Object.freeze({
  ...
  "facet-inspect": z.strictObject({ ...base, ...pageOutput, facet: facetSchema, zoneId: identifierSchema, zoneName: z.string(), observed: z.strictObject({ status: z.enum(["present", "absent", "unknown"]), comparison: boundedValueSchema.nullable(), inspection: boundedValueSchema.nullable() }), capabilities: z.array(z.string()), actions: boundedValueSchema.nullable(), intent: z.strictObject({ revision: z.string(), status: z.string(), reason: z.string().nullable(), conflictKinds: z.array(z.string()), acknowledgementCount: z.number().int().nonnegative() }), items: z.array(z.strictObject({ policy: policySummarySchema, effective: z.boolean(), targeted: z.boolean(), overriddenBy: z.array(z.string()), status: z.string().nullable().describe("Effective observed policy result; unresolved when the facet is unavailable, unknown when required reads are incomplete"), reason: z.string().nullable(), acknowledgements: boundedValueSchema })) }),
})
