...
The local CLI and stdio MCP server inherit the Cloudflare API token directly and therefore belong inside the trusted operator boundary. They expose named redacted diagnosis, audit, complete intent persistence, alignment, bounded direct change, activity, and guarded undo operations rather than arbitrary API requests. Every apply binds the exact request and freshly regenerated plan digest, serializes local mutations, and keeps the token out of tool inputs and outputs. MCP apply additionally requires authenticated short-lived confirmation state plus explicit approval of the exact displayed request and plan. Intent persistence requires the current account and revision. Guarded undo compares live state with recorded verification evidence before review and again before inverse execution. Short-lived alignment baseline reuse is bound to the intent revision and never replaces fresh membership and selected-surface reads. Throttled GET requests back off, exhausted throttling fails closed, and mutation requests are not automatically replayed. Keep agent configuration, state, audit reports, plans, activity, and process output private.
